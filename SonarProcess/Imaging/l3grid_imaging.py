...
def l3grid_imaging(grd_in,grd_out):
    x = grd_in['x'].reshape(-1)
    y = grd_in['y'].reshape(-1)
    z = grd_in['z'].reshape(-1)
    xg1 = grd_out['x']
    yg1 = grd_out['y']
    zg1 = grd_out['z']
    ...
    Ia = grd_in['Ia'].reshape(-1)
    Id = grd_in['Id'].reshape(-1)
    Ia_filt = grd_in['Ia_filt'].reshape(-1)
    Id_filt = grd_in['Id_filt'].reshape(-1)
    Kp = grd_in['Kp'].reshape(-1)

    # use only finite points within the grid
    ii = np.where(np.isfinite(Id)&np.isfinite(Ia)&(x>=xmin)&(x<=xmax)&(y>=ymin)&(y<=ymax)&(z>=zmin)&(z<=zmax))[0]
