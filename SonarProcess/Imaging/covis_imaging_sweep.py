...
def covis_imaging_sweep(filedir, json_file='F:/Covis/pythonProject/Inputs/covis_image.json'):
    ...
    exit(0)
    '''
    s = build_structured_grid(xv_out, yv_out, zv_out, array_dict={
        'Ia': Ia_out,
        'Id': Id_out,
        'Ia_filt': Ia_filt_out,
        'Id_filt': Id_filt_out,
    ...
    save_structured_grid(s, "F:/data/", "2011s.vtk")

    exit(0)

    # ss = get_isosurface(s, 0.002, 'Ia_filt')
    #
    # save_isosurface(ss, "F:/data/", "2011ss.vtp")
    ...
    i = build_image_grid(array_dict={
        'Ia': grd_out['Ia'],
        'Id': grd_out['Id'],
        'Ia_filt': grd_out['Ia_filt'],
        'Id_filt': grd_out['Id_filt'],
        'Kp': grd_out['Kp']
    }, )

    save_image(i, "F:/data/", "2011i.vti")

    # si = get_isosurface(i, 0.002, 'Ia_filt')
    #
    # save_isosurface(si, "F:/data/", "2011si.vtp")
...
def parse_covis_json(name, json_file='F:/Covis/pythonProject/Inputs/covis_image.json'):
    # parsing the json input file for the user supplied parameters
    if not os.path.exists(json_file):
        print('Error: JSON input file does not exist.')
        return
    ...
    # define a 3D rectangular data grid
    covis['grid'] = covis_rectgrid_imaging(covis['grid'])
    covis['grid']['name'] = name
    grd_out = covis['grid']
    print(grd_out)


def covis_imaging_origin_format(filedir, covis: dict):
    head_tail = os.path.split(filedir)
    swp_path = head_tail[0]
    swp_name = head_tail[1]
...
def covis_imaging_unigrid_resample(grid_in, grid_out: dict):
    grid_out = l3grid_imaging(grid_in, grid_out)

    # normalize the grid with the grid weights
    grid_out['Ia'] = np.divide(grid_out['Ia'], grid_out['w'], where=grid_out['w'] != 0)
    grid_out['Id'] = np.divide(grid_out['Id'], grid_out['w'], where=grid_out['w'] != 0)
    grid_out['Ia_filt'] = np.divide(grid_out['Ia_filt'], grid_out['w'], where=grid_out['w'] != 0)
    grid_out['Id_filt'] = np.divide(grid_out['Id_filt'], grid_out['w'], where=grid_out['w'] != 0)
    grid_out['Kp'] = np.divide(grid_out['Kp'], grid_out['w'], where=grid_out['w'] != 0)
